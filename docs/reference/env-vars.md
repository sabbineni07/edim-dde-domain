...
EDIM_CHECKPOINTER         →  LangGraph session checkpoints (multi-turn analysis)
EDIM_RECOMMENDATION_STORE →  recommend / RCA history rows  (product history)
```

| Question | `EDIM_STATE_STORE` | `EDIM_CHECKPOINTER` | `EDIM_RECOMMENDATION_STORE` |
|----------|--------------------|---------------------|------------------------------|
| What is it? | Platform catalog + HITL session/audit | Graph state + in-thread messages | Durable outcomes from agent HTTP calls |
...
```bash
# Usual: control plane + history on the same backend
